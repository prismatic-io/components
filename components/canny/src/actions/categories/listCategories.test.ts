...
describe("listCategories", () => {
  ...
  test("returns one page of categories for a board", async () => {
    const { captured, matcher } = captureBody();
    nock(CANNY_HOST)
      .post(PATH, matcher)
      .reply(200, listCategoriesExamplePayload.data);
    const { result } = await invoke(listCategories, params);
    ...
    expect(result.data).toStrictEqual(listCategoriesExamplePayload.data);
  });
  test("surfaces an API error", async () => {
    nock(CANNY_HOST).post(PATH).reply(400, { error: "something went wrong" });
    await expect(invoke(listCategories, params)).rejects.toThrow();
  });
});
