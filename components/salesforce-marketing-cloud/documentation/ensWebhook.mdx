...
2. When ENS sends the verification request to the webhook URL, this trigger automatically captures the verification key.
...
- **Signature Validation**: ENS event payloads include an HMAC-SHA256 signature in the `X-SFMC-ENS-Signature` or `X-SFMC-HMAC-SHA256` header. When the signature key (returned during callback creation) is provided, this trigger automatically validates the signature to ensure events originate from Salesforce Marketing Cloud.
