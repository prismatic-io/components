...
For `propertyChange` events, individual property names can optionally be specified to monitor (e.g., only trigger when the `email` or `phone` property changes).

A full list of available webhook events and required OAuth scopes is available in the [HubSpot Webhooks API documentation](https://developers.hubspot.com/docs/api-reference/legacy/webhooks/webhooks-journal).
