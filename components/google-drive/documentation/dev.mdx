...
Either run **List Changes** on a schedule with a [schedule trigger](components/schedule-triggers.mdx) (for example, check for changes every 5 minutes), or use Google Drive's [change API](https://developers.google.com/drive/api/guides/push) to receive notifications in real time when changes occur.
