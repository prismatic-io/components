...
import { leadsPollingTrigger } from "./crmLeads";
const conn = {
  ...createConnection(
...
// biome-ignore lint/suspicious/noExplicitAny: test-only context/params bridge
...
const trigger: any = leadsPollingTrigger;
describe("leadsPollingTrigger", () => {
  afterEach(() => nock.cleanAll());
  test("threads state across polls: first poll sends no If-Modified-Since, second poll sends toZohoTimestamp(lastUpdated)", async () => {
    const store: {
      lastUpdated?: string;
    } = {};
