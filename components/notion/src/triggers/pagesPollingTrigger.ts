...
export const pagesPollingTrigger = pollingTrigger({
  ...
  triggerResolver: {
    resolveItems: (_context, result) => {
      const data = result.payload.body.data as PagesChangesObject | undefined;
      return resolveRecordChanges(data?.newPages, data?.updatedPages);
    },
  ...
  perform: async (context, payload, params) => {
    ...
    if (context.debug.enabled) {
      context.logger.debug(
        `Polled Notion pages from ${windowStart}: ${round.records.length} walked, ${created.length} new, ${updated.length} updated${nextState ? ", more pages outstanding" : ""}`,
      );
    }
  ...
  onDeployPerform: async (context, payload, params) => {
    ...
    if (!incoming && context.instanceState?.[BACKFILL_COMPLETED_KEY]) {
      context.logger.debug(
        "Notion page initial sync already completed for this instance; skipping.",
      );
      return {
        payload: {
          ...payload,
          body: { data: { newPages: [], updatedPages: [] } },
        },
    ...
    context.logger.info(
      `Notion page initial sync from ${windowStart}: ${round.records.length} walked${round.nextCursor ? ", more pages outstanding" : ", backfill complete"}`,
    );
    if (round.nextCursor) {
      context.logger.error(
        `Notion page initial sync stopped at its ${DEFAULT_BACKFILL_MAX_PAGES} page limit with more pages outstanding. The first scheduled poll is pointed back at ${windowStart}, so nothing is skipped and what the sync already sent is sent again. Narrow the Look-back Date to avoid this.`,
      );
    }
    return {
      payload: {
